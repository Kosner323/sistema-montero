...
def validate_request(model: Type[BaseModel], source: str = "json", location: str = "body") -> Callable:
    """
...
def _get_request_data(source: str) -> dict:
    ...
    if source == "json":
        if not request.is_json:
            raise ValueError("Se esperaba JSON pero el content-type no es application/json")
        ...
        raise ValueError(f"Fuente '{source}' no soportada. Use: json, form, args, o auto")
...
def _format_validation_errors(validation_error: ValidationError) -> list:
    ...
    for error in validation_error.errors():
        ...
        if error_type == "missing":
            ...
            ctx = error.get("ctx", {})
            message = f"El campo '{location}' debe ser mayor o igual a {ctx.get('ge', 'N/A')}"
...
def validate_manual(model: Type[BaseModel], data: dict) -> tuple[Optional[BaseModel], Optional[list]]:
    """
