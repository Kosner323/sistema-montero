...
class PreRequisitesChecker:
    ...
    def check_modules(self):
        ...
        for module_name, description in modules_to_check:
            try:
                ...
                self.add_check(
                    f"Módulo: {module_name}", "ok", f"Disponible ({description})"
                )
            except ImportError as e:
                self.add_check(
                    f"Módulo: {module_name}", "error", f"No encontrado - {str(e)}"
                )
                all_ok = False
        return all_ok
    ...
    def check_encryption_key(self):
        ...
        if os.path.exists(env_path):
            try:
                ...
                self.add_check(
                    "ENCRYPTION_KEY", "error", f"Error leyendo {env_path}: {e}"
                )
                return False
    ...
    def check_database(self):
        ...
        if os.path.exists(db_path):
            try:
                ...
                cur = conn.cursor()
                cur.execute(
                    "SELECT name FROM sqlite_master WHERE type='table' AND name='credenciales_plataforma'"
                )
                ...
                self.add_check(
                    "Base de datos", "error", f"Error accediendo a {db_path}: {e}"
                )
        else:
            self.add_check(
                "Base de datos", "error", f"No se encontró mi_sistema.db en {db_path}"
            )
        return False, None, 0
    ...
    def check_backup_directory(self):
        ...
        if backup_dir.exists():
            ...
            try:
                backup_dir.mkdir()
                self.add_check(
                    "Directorio de respaldos", "ok", "backups/ creado exitosamente"
                )
                return True
            except Exception as e:
                self.add_check(
    ...
    def check_disk_space(self):
        try:
            ...
            if free_space_mb > 100:
                self.add_check(
                    "Espacio en disco", "ok", f"{free_space_mb:.0f} MB disponibles"
                )
                return True
            else:
                self.add_check(
    ...
    def check_permissions(self):
        ...
        try:
            ...
            self.add_check(
                "Permisos de escritura", "error", f"No se puede escribir: {e}"
            )
            return False
