    ...
    try:
        ...
        if not any(issues.values()):
            ...
            if issues["empleados_huerfanos"]:
                print_error(
                    f"Empleados huérfanos: {len(issues['empleados_huerfanos'])}"
                )
            ...
            if issues["documentos_duplicados"]:
                print_error(
                    f"Documentos duplicados: {len(issues['documentos_duplicados'])}"
                )
        ...
        if critical_issues == 0 and data_issues == 0:
            ...
            print_error(
                f"⚠️ CRÍTICO - {critical_issues} problemas de seguridad encontrados"
            )
        else:
