...
def validate_credentials():
    ...
    if not db_path:
        print(
            f"❌ No se encontró la base de datos en: {os.path.join(PROJECT_ROOT, 'data')}"
        )
        return False
    ...
    try:
        ...
        for cred in credenciales:
            ...
            try:
                ...
                fallidas += 1
                resultados.append(
                    [cred_id, plataforma[:30], tipo[:15], "Error", "Error", estado]
                )
        ...
        for i, original in enumerate(test_cases, 1):
            try:
                ...
                if decrypted == original:
                    print(
                        f"  ✅ Test {i}: OK - '{original}' → [encrypted] → '{decrypted}'"
                    )
                else:
                    print(
                        f"  ❌ Test {i}: FALLO - Original: '{original}' | Recuperado: '{decrypted}'"
                    )
                    all_passed = False
            except Exception as e:
...
def main():
    ...
    print(
        f"  🧪 Sistema de encriptación:        {'✅ OK' if encryption_ok else '❌ Fallo'}"
    )
    print(
        f"  💾 Credenciales en BD:              {'✅ OK' if credentials_ok else '❌ Con errores'}"
    )
    print("=" * 80)
