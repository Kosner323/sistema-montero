...
try:
    ...
    from encryption import decrypt_text, encrypt_text, get_encryption
...
class CredentialMigrator:
    ...
    def analyze_database(self):
        ...
        try:
            ...
            for cred in credenciales:
                ...
                usuario_enc = self.is_encrypted(cred["usuario"]) if cred["usuario"] else True
                contra_enc = self.is_encrypted(cred["contrasena"]) if cred["contrasena"] else True

                estado = "🔒 ENCRIPTADA" if (usuario_enc and contra_enc) else "🔓 TEXTO PLANO"
        ...
        try:
            ...
            for cred in credenciales:
                try:
                    ...
                    nuevo_usuario = encrypt_text(usuario) if usuario and not usuario_enc else usuario
                    nueva_contrasena = encrypt_text(contrasena) if contrasena and not contra_enc else contrasena
                    ...
                    print(f"  ✅ ID {cred_id:3d} | {plataforma:30s} | Encriptada correctamente")
            ...
            for cred in credenciales:
                try:
                    ...
                    if usuario_dec and contra_dec:
                        print(f"  ✅ ID {cred_id:3d} | {plataforma:30s} | Verificada OK")
                        verificadas += 1
                    else:
                        print(f"  ⚠️  ID {cred_id:3d} | {plataforma:30s} | Verificación parcial")
                        errores_verificacion += 1

                except Exception as e:
                    print(f"  ❌ ID {cred_id:3d} | {plataforma:30s} | Error verificando: {e}")
                    errores_verificacion += 1
    ...
    def print_summary(self):
        ...
        if self.stats["errores"] == 0:
            ...
            logger.info(f"Migración exitosa: {self.stats['migradas']} credenciales encriptadas")
        else:
...
def main():
    ...
    try:
        ...
        if not migrator.create_backup():
            if input("\n⚠️  No se pudo crear respaldo. ¿Continuar de todos modos? (s/n): ").lower() != "s":
