...
TABLES_SCHEMA = {
    ...
    "empresas": {
        ...
        "relaciones": ["Tiene relación 1:N con 'usuarios' (una empresa puede tener múltiples empleados)"],
        "columnas": {
            "id": {
                "tipo": "INTEGER",
                "nullable": True,
    ...
    for table_name, table_info in TABLES_SCHEMA.items():
        for col_name, col_info in table_info.get("columnas", {}).items():
            if "foreign_key" in col_info:
                ...
                fk_relations[table_name].append({"columna": col_name, "referencia": col_info["foreign_key"]})
    return fk_relations
