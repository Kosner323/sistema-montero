...
def print_header():
    ...
    print(
        f"{Colors.BOLD}{Colors.CYAN}{'VALIDACIÓN DE CONFIGURACIÓN DE ALEMBIC':^70}{Colors.END}"
    )
...
def check_sqlalchemy_installed():
    ...
    try:
        ...
        print_check(
            "SQLAlchemy instalado", False, "Instalar con: pip install sqlalchemy"
        )
        return False
...
def check_database():
    ...
    try:
        ...
        if has_alembic_table:
            ...
            if version:
                print_check(
                    "Estado de migración", True, f"Versión actual: {version[0]}"
                )
            else:
                print_check("Estado de migración", True, "Sin versión (BD nueva)")
        else:
            print_check(
                "Tabla de versiones Alembic", False, "BD no inicializada con Alembic"
            )
...
def print_summary(checks_passed):
    ...
    if passed == total:
        ...
        print(f"\nComandos útiles:")
        print(
            f"  • Ver estado: {Colors.CYAN}python manage_migrations.py status{Colors.END}"
        )
        ...
        if not checks_passed[1]:  # SQLAlchemy
            print(
                f"2. Instalar SQLAlchemy: {Colors.CYAN}pip install sqlalchemy{Colors.END}"
            )
        if not checks_passed[2]:  # Estructura
            print(
                f"3. Crear estructura: {Colors.CYAN}alembic init migrations{Colors.END}"
            )
        print(
