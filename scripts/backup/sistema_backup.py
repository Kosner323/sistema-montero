...
class SistemaBackup:
    ...
    def __init__(self, dir_proyecto: str = None, dir_backups: str = None, dias_retencion: int = 30):
        """
    ...
    def listar_backups(self) -> List[dict]:
        ...
        for archivo in sorted(self.dir_backups.glob("backup_*.zip"), reverse=True):
            try:
                ...
                backups.append({"nombre": archivo.name, "ruta": archivo, "tamano_mb": tamano_mb, "fecha": fecha_mod})
            except Exception as e:
...
class RestauradorBackup:
    ...
    def restaurar_backup(self, archivo_backup: str, dir_destino: str = None, sobrescribir: bool = False) -> bool:
        """
...
Ejemplos de uso:
  ...
  python sistema_backup.py --restore latest   # Restaurar último backup
    ...
    parser.add_argument("--full", action="store_true", help="Crear backup completo del proyecto")
    ...
    parser.add_argument("--restore", metavar="BACKUP", help='Restaurar un backup específico (nombre o "latest")')
    parser.add_argument("--dir-backups", metavar="DIR", help="Directorio personalizado para backups")
    parser.add_argument("--retention", type=int, default=30, help="Días de retención de backups (default: 30)")

    args = parser.parse_args()

    # Crear sistema de backup
    sistema = SistemaBackup(dir_backups=args.dir_backups, dias_retencion=args.retention)
