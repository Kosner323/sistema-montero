...
try:
    from utils import (
        ...
        login_required,
        format_key,
        # USER_DATA_FOLDER, # <-- Eliminada de la importación
...
bp_usuarios = Blueprint("bp_usuarios", __name__, url_prefix="/api/usuarios")
...
def add_usuario():
    conn = None
    numero_id = "desconocido" # Definir por si falla antes de leerlo del form
    try:
        data = request.form
        ...
        logger.info(f"Carpeta creada para usuario: {numero_id} en {user_folder_path}") # Log actualizado para verificar ruta
        ...
        for key, value in data.items():
            ...
            if key.startswith(
                ("nombreBeneficiario_", "parentesco_", "documentoBeneficiario_")
            ):
                parts = key.split("_")
                if len(parts) == 2:
        ...
        with open(
            os.path.join(user_folder_path, "datos_usuario.txt"), "w", encoding="utf-8"
        ) as f:
        ...
        if firma_data_url and "," in firma_data_url:
            try:
                ...
                if len(firma_data) <= 2 * 1024 * 1024:  # Validar tamaño (máximo 2MB)
                    with open(
                        os.path.join(user_folder_path, "firma_usuario.png"), "wb"
                    ) as f:
                        f.write(firma_data)
                ...
                log_file_upload(
                    "firma_usuario.png", user_session_id, success=False, error=str(e)
                )
                ...
                if is_valid:
                    try:
                        ...
                        filepath = sanitize_and_save_file(
                            pdf_file, user_folder_path, custom_name
                        )
                        saved_files["cedula"] = os.path.basename(filepath)
                        log_file_upload(
                            pdf_file.filename, user_session_id, success=True
                        )
        ...
        os.makedirs(
            os.path.join(user_folder_path, "RECIBOS", "RECIBOS DE CAJA"), exist_ok=True
        )
        os.makedirs(
        ...
        try:
            ...
            logger.info(
                f"Usuario {numero_id} guardado exitosamente por user_id: {user_session_id}"
            )
            ...
            if upload_errors:
                ...
                logger.warning(
                    f"Usuario {numero_id} creado con {len(upload_errors)} errores de carga: {upload_errors}"
                )
        ...
        logger.error(
            f"Error general en add_usuario (ID: {numero_id}): {e}", exc_info=True
        )
...
def get_usuarios():
    ...
    try:
        ...
        if empresa_nit:
            ...
            usuarios = conn.execute(
                "SELECT * FROM usuarios ORDER BY primerNombre, primerApellido"
            ).fetchall()

        logger.debug(
            f"Se consultaron {len(usuarios)} usuarios (filtro NIT: {empresa_nit})"
        )
...
@bp_usuarios.route("/buscar", methods=["GET"])
@login_required
def buscar_usuario():
    """
