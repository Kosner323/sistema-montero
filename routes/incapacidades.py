...
from functools import wraps  # Necesario para el fallback de login_required
...
try:
    ...
    logger.error(f"Error importando utils en incapacidades.py: {e}", exc_info=True)
...
bp_incapacidades = Blueprint(
    "bp_incapacidades", __name__, url_prefix="/api/incapacidades"
)
...
def _get_user_incapacidad_folder(numero_id):
    ...
    try:
        user_folder_path = os.path.join(
            USER_DATA_FOLDER, str(numero_id), "INCAPACIDADES"
        )
        os.makedirs(user_folder_path, exist_ok=True)
        return user_folder_path
    except Exception as e:
        logger.error(
            f"Error creando carpeta de incapacidad para {numero_id}: {e}", exc_info=True
        )
        raise
...
def add_incapacidad():
    ...
    try:
        ...
        if not all(field in data for field in required_fields):
            logger.warning(
                f"Intento de agregar incapacidad con campos faltantes. Usuario: {numero_id}"
            )
            return (
                jsonify(
                    {
                        "error": "Faltan campos obligatorios (usuario_id, tipo, fecha_inicio, dias)."
                    }
                ),
                400,
            )
        ...
        if "soporte_pdf" not in request.files:
            logger.warning(
                f"Intento de agregar incapacidad sin soporte PDF. Usuario: {numero_id}"
            )
        ...
        if not is_valid:
            logger.warning(
                f"Archivo de incapacidad inválido para {numero_id}: {error_msg}"
            )
        ...
        try:
            ...
            custom_name = f"incapacidad_{data['tipo_incapacidad']}_{data['fecha_inicio']}_{ts}.pdf".replace(
                " ", "_"
            )
        ...
        try:
            ...
            cur.execute(
                ...
                INSERT INTO incapacidades (
                    ...
                    diagnostico, eps_arl, ruta_soporte_pdf, estado
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            """,
                (
                    numero_id,
            ...
            nuevo_registro = conn.execute(
                "SELECT * FROM incapacidades WHERE id = ?", (nuevo_id,)
            ).fetchone()
