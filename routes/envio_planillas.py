    ...
    try:
        ...
        if not all(field in data for field in required_fields):
            ...
            return (
                jsonify(
                    {
                        "error": "Faltan campos obligatorios (empresa_nit, fecha_envio, plataforma, estado)."
                    }
                ),
                400,
            )
        ...
        logger.info(
            f"Nuevo envío de planilla registrado con ID: {nuevo_id} para NIT: {data['empresa_nit']}"
        )

        nuevo_envio = conn.execute(
            "SELECT * FROM envios_planillas WHERE id = ?", (nuevo_id,)
        ).fetchone()
