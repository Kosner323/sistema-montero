...
from flask import (
    ...
    jsonify,
    session,
)  # Añadir session si necesitas el user_id
    ...
    try:
        ...
        for row in novedades:
            ...
            try:
                ...
                novedad_dict["beneficiaries"] = (
                    json.loads(beneficiaries_str) if beneficiaries_str else []
                )
...
def create_novedad():
    ...
    try:
        ...
        now_dt = datetime.now()
        now_str_datetime = now_dt.strftime(
            "%Y-%m-%d %H:%M:%S"
        )  # Fecha y hora para historial
        now_str_date = now_dt.strftime(
            "%Y-%m-%d"
        )  # Solo fecha para creation/update date
        ...
        """,
            (
                ...
                data.get("status"),
                data.get(
                    "priorityText", data.get("priority")
                ),  # Usar texto de prioridad
                data.get("idType"),
                data.get("idNumber"),
                data.get("firstName"),
                data.get("lastName"),
        ...
        novedad_creada_row = conn.execute(
            "SELECT * FROM novedades WHERE id = ?", (new_id,)
        ).fetchone()
        ...
        novedad_creada_dict["beneficiaries"] = json.loads(
            novedad_creada_dict.get("beneficiaries") or "[]"
        )
        novedad_creada_dict["history"] = json.loads(
            novedad_creada_dict.get("history") or "[]"
        )
...
def update_novedad(novedad_id):
    ...
    try:
        ...
        novedad_actual_row = conn.execute(
            "SELECT * FROM novedades WHERE id = ?", (novedad_id,)
        ).fetchone()
        ...
        try:
            ...
            logger.info(
                f"Advertencia: Historial inválido para novedad {novedad_id}. Iniciando historial nuevo."
            )
            current_history = []
        ...
        updated_fields = (
            {}
        )  # Diccionario para guardar los campos SQL que realmente cambian
        action_description_parts = (
            []
        )  # Lista para construir la descripción de la acción
            ...
            if key in data and data[key] != novedad_actual_dict.get(key):
                ...
                if key == "beneficiaries":
                    ...
                    action_description_parts.append(
                        f"Actualizó beneficiarios."
                    )  # Descripción genérica
                # Caso especial para IBC: convertir a float
                elif key == "ibc":
                    try:
                        ibc_float = (
                            float(data[key]) if data[key] not in [None, ""] else None
                        )
                        if ibc_float != novedad_actual_dict.get(
                            key
                        ):  # Comparar después de convertir
                            updated_fields[key] = ibc_float
                            action_description_parts.append(
                                f"Actualizó IBC a {ibc_float}."
                            )
                    except (ValueError, TypeError):
                        print(
                            f"Advertencia: valor IBC inválido '{data[key]}' no se actualizará."
                        )
                # Otros campos
                else:
                    ...
                    if key == "status":
                        action_description_parts.append(
                            f"Cambió estado a '{data[key]}'."
                        )
                    elif key == "priority":
                        action_description_parts.append(
                            f"Cambió prioridad a '{data.get('priorityText', data[key])}'."
                        )
                    elif key == "assignedTo":
        ...
        if updated_fields or new_comment:
            action_description = (
                ...
                if action_description_parts
                else "Actualizó el caso."
            )
            ...
            updated_fields["history"] = json.dumps(
                current_history
            )  # Guardar historial actualizado
            updated_fields["updateDate"] = (
                now_str_date  # Actualizar fecha de modificación
            )
            ...
            novedad_actualizada_row = conn.execute(
                "SELECT * FROM novedades WHERE id = ?", (novedad_id,)
            ).fetchone()
            novedad_actualizada_dict = dict(novedad_actualizada_row)
            # Convertir JSON strings de vuelta a listas/objetos para la respuesta
            novedad_actualizada_dict["beneficiaries"] = json.loads(
                novedad_actualizada_dict.get("beneficiaries") or "[]"
            )
            novedad_actualizada_dict["history"] = json.loads(
                novedad_actualizada_dict.get("history") or "[]"
            )
        ...
        conn.rollback()
        logger.error(
            f"Error de base de datos al actualizar novedad {novedad_id}: {db_err}"
        )
        # traceback.print_exc()  # MIGRADO: reemplazar por logger.error con exc_info=True
        return jsonify({"error": f"Error de base de datos: {str(db_err)}"}), 500
...
def delete_novedad(novedad_id):
    ...
    try:
        ...
        conn.rollback()
        logger.error(
            f"Error de base de datos al eliminar novedad {novedad_id}: {db_err}"
        )
        # traceback.print_exc()  # MIGRADO: reemplazar por logger.error con exc_info=True
        return jsonify({"error": f"Error de base de datos: {str(db_err)}"}), 500
    except Exception as e:
