    ...
    try:
        ...
        if not all(field in data for field in required_fields):
            ...
            return (
                jsonify(
                    {
                        "error": "Faltan campos obligatorios (empresa_nit, monto, servicio)."
                    }
                ),
                400,
            )
        ...
        logger.info(
            f"Nueva cotización registrada con ID: {nuevo_id} para empresa {data['empresa_nit']}"
        )

        # Devolver el registro completo
        nuevo_cotizacion = conn.execute(
            "SELECT * FROM cotizaciones WHERE id = ?", (nuevo_id,)
        ).fetchone()
        ...
        return (
            jsonify(
                {
                    "error": "Error de integridad, verifique que el NIT de la empresa exista."
                }
            ),
            409,
        )
