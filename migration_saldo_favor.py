...
# -*- coding: utf-8 -*-
"""
migration_saldo_favor.py
========================
Migración: Agregar columna 'saldo_a_favor' a la tabla empresas
...
def verificar_columna_existe(cursor):
    """Verifica si la columna 'saldo_a_favor' ya existe"""
    cursor.execute("PRAGMA table_info(empresas)")
    columnas = [col[1] for col in cursor.fetchall()]
    return 'saldo_a_favor' in columnas


def ejecutar_migracion():
    """Ejecuta la migración de la columna saldo_a_favor"""
    print("\n" + "="*80)
    print("MIGRACIÓN: Agregar columna 'saldo_a_favor' a tabla empresas")
    print("="*80)
    ...
    try:
        ...
        print("\n📋 Verificando si columna 'saldo_a_favor' ya existe...")

        if verificar_columna_existe(cursor):
            print("⚠️  La columna 'saldo_a_favor' ya existe en la tabla empresas")
        ...
        # PASO 1: Agregar columna saldo_a_favor
        print("\n📋 PASO 1: Agregando columna 'saldo_a_favor' a tabla empresas...")

        cursor.execute("""
            ALTER TABLE empresas
            ADD COLUMN saldo_a_favor REAL DEFAULT 0.0
        """)

        print("   ✅ Columna 'saldo_a_favor' agregada exitosamente")

        # PASO 2: Actualizar registros existentes
        print("\n📋 PASO 2: Inicializando registros existentes con saldo 0...")

        cursor.execute("""
            UPDATE empresas
            SET saldo_a_favor = 0.0
            WHERE saldo_a_favor IS NULL
        """)

        filas_actualizadas = cursor.rowcount
        print(f"   ✅ {filas_actualizadas} empresas inicializadas con saldo_a_favor = 0.0")

        # PASO 3: Verificar cambios
        print("\n📋 PASO 3: Verificando cambios...")

        cursor.execute("SELECT COUNT(*) FROM empresas WHERE saldo_a_favor = 0.0")
        total_saldo_cero = cursor.fetchone()[0]

        cursor.execute("SELECT COUNT(*) FROM empresas")
        total_empresas = cursor.fetchone()[0]

        print(f"   ✅ Total empresas: {total_empresas}")
        print(f"   ✅ Empresas con saldo_a_favor = 0.0: {total_saldo_cero}")
        ...
        print("="*80)
        print("\nRESUMEN:")
        print(f"  ✓ Columna 'saldo_a_favor' agregada a tabla empresas")
        print(f"  ✓ {filas_actualizadas} registros inicializados")
...
if __name__ == "__main__":
    print("\n" + "="*80)
    print(" "*20 + "FASE 10.4: SALDO A FAVOR")
    print("="*80)
    ...
    if resultado:
        print("\n[RESULTADO FINAL] Migración exitosa - Sistema listo para gestión de excedentes")
    else:
